...
pub struct Server {
    ...
    ///
    /// Please refer to the documentation for authentication to see its documentation.
    #[config(as_struct, as_type = "PartialAuth")]
...
impl Server {
    fn from_partial(
        ...
        let mut auth = Auth::from_partial(partial.auth)
            .change_context(ServerLoadError)
            .attach_printable("could not load authentication configuration")?;

        // resolving public and private key locations
        let jwt_dir_location: Option<PathBuf> = file_location
            .as_ref()
            .and_then(|v| v.parent().map(|v| v.to_path_buf()))
            .or_else(|| vfs.current_dir().ok());

        if let Some(jwt_dir_location) = jwt_dir_location {
            if auth.jwt.private_key_file.is_relative() {
                auth.jwt.private_key_file =
                    vfs.join_path(jwt_dir_location, auth.jwt.private_key_file);
            }
        }
...
impl PartialServer {
    pub fn from_env() -> Result<Self, ServerLoadError> {
        let auth = PartialAuth::from_env().change_context(ServerLoadError)?;
