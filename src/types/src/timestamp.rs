...
use std::{fmt::Display, hash::Hash, num::NonZeroU64, ops::Deref, str::FromStr};
use thiserror::Error;

// Whim epoch starts at November 18, 2023 at 07:52:48 AM in Manila time
const EPOCH: u64 = 1700265168293;
...
impl Into<chrono::ParseError> for ParseError {
  fn into(self) -> chrono::ParseError {
    self.0
  }
}
...
mod tests {
  ...
  fn test_from_snowflake() {
    ...
    let timestamp = Timestamp::from_snowflake(
      (23400000055 << (63 - TIMESTAMP_BITS_LEN))
        .try_into()
        .unwrap(),
    );
