...
impl<T: Marker> Debug for Id<T> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    static MARKER_MODULE: Lazy<String> = Lazy::new(|| {
      format!("{}::id::marker::", env!("CARGO_PKG_NAME").to_snake_case())
    });
    ...
    impl<'de, T: Marker> serde::de::Visitor<'de> for Visitor<T> {
        ...
        E: DeError,
      {
        let value = u64::try_from(v).map_err(|_| {
          DeError::invalid_value(Unexpected::Signed(v), &"nonzero u64")
        })?;
        ...
        E: serde::de::Error,
      {
        let value = NonZeroU64::new(v).ok_or_else(|| {
          DeError::invalid_value(Unexpected::Unsigned(v), &"nonzero u64")
        })?;
...
mod tests {
  ...
  #[cfg(not(feature = "server_impl"))]
  assert_impl_all!(Id<AnyMarker>:
    Debug, Display, Clone, Copy, Send, Sync, Hash
  );
  ...
  fn test_fmt_display_impl() {
    assert_eq!("1234567890", Id::<AnyMarker>::new(1_234_567_890).to_string());
  }
