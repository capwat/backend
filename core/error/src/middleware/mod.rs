...
type StoredMiddleware =
    Arc<dyn Fn(Box<dyn Context + 'static>, Location<'static>) -> Report + Send + Sync>;
...
impl Error {
    ...
    pub fn install_middleware<T: Context + Any + Send + Sync + 'static>(
        middleware: impl Fn(T, Location<'static>) -> Report + Send + Sync + 'static,
    ) {
...
fn into_boxed_middleware<T: Context + Any + 'static>(
    ...
    Arc::new(
        #[track_caller]
        move |error: Box<dyn Context>, location: Location<'_>| {
